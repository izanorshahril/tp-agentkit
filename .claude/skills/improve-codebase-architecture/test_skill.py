...
class ImproveCodebaseArchitectureTests(SkillTestCase):
    def test_skill_contract_is_behavior_only(self) -> None:
        text = self.read_skill_markdown("improve-codebase-architecture")
        skill_dir = SKILLS_ROOT / "improve-codebase-architecture"
        executable_files = {
            path.name
        ...
        self.assertIn("deep-module opportunities", text)
        self.assertIn("local artifact, not a GitHub issue", text)
        self.assertIn("design-an-interface", text)
