...
class UbiquitousLanguageTests(SkillTestCase):
    def test_skill_contract_is_behavior_only(self) -> None:
        text = self.read_skill_markdown("ubiquitous-language")
        skill_dir = SKILLS_ROOT / "ubiquitous-language"
        executable_files = {
            path.name
        ...
        self.assertIn("Do not write a root-level `UBIQUITOUS_LANGUAGE.md`", text)
        self.assertIn("flagged ambiguities", text)
        self.assertIn(".claude/knowledge/", text)
